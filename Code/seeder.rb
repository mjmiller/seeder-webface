require "t2seeder"
 ...
 ###HERE IS WHERE I AM TRYING COLORS @lexs=["red","blue","green","orange","violet","brown"]
        #
                        ...
                        synset.to_hash.each do  |key,value|
                          ...
                          if !(key.to_s == "definition")
                            if (key.to_s == "pos")
                              ...
                              th_def.update_attribute(:th_part_of_speech_id, ThPartOfSpeech.find_by_name(pos).id)
                              #TODO: Add pos code
                            else
                            key_mod_info = ThModInfo.create(:th_source_id => synset_src.id, :th_editor_action_id => MyEditAction.id)
                            meta_data_key = ThMetadataKey.new(:key => key,:description => "some description",:th_mod_info_id => key_mod_info.id)
                            # :value => value.to_s, :th_phrase_definition_id=> th_phr_definition.id)
                            meta_data_key.save 
                            puts "metadata key saved"
